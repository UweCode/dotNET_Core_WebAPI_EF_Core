        ...
        public async Task<ServiceResponse<AttackResultDto>> SkillAttack(SkillAttackDto request)
        {
            ...
            try
            {
                ...
                CharacterSkill characterSkill =
                    attacker.CharacterSkills.FirstOrDefault(cs => cs.SkillId == request.SkillId);
            ...
            try
            {
                ...
                while(!defeated)
                {
                    foreach (Character attacker in characters)
                    {
                        ...
                        Character opponent = opponents[new Random().Next(opponents.Count)]; // using a random number for the index of the opponents list

                        int damage = 0;
                        string attackUsed = string.Empty; // ... to log the Weapon or Skill for each attack

                        bool useWeapon = new Random().Next(2) == 0; // attacker used a Weapon or a Skill?
                        if (useWeapon)
                        {
